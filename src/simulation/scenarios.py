...
class SimulationScenarios:
    ...
    def _er_ventilation_temperature(self, t: float) -> Dict[str, float]:
        ...
        Phase 3 (150-300초, 2.5분): 60Hz 유지 → 정상 대수 증설
        ...
        Phase 4 (300-390초, 1.5분): 극한 근접 → 선제 증설 테스트
            - T6: 44.5°C → 46°C (극한 근접)
            - ML 예측: 47°C 예상!
            - 예상: Priority 2 발동! (극한 예상 → 즉시 증설)
            - 만약 아직 3대면 → 4대, 이미 4대면 유지
            - 목적: ⭐ ML 예측 기반 선제 대응 확인
        
        Phase 5 (390-480초, 1.5분): 온도 하강 (46→43°C)
            - T6: 46°C → 43°C (목표 복귀)
            - 예상: 피드백 제어 감속, 4대 유지
        
        Phase 6 (480-570초, 1.5분): 저온 진입 (43→40°C)
            - T6: 43°C → 40°C (저온)
            - 예상: 40Hz 도달, 4대 유지
        
        Phase 7 (570-660초, 1.5분): 40Hz 유지 → 4대→3대 감소
            - T6: 40°C 유지
            - 예상: 40Hz 10초 → 4대 → 3대 감소
            - 주파수: 40Hz → 48Hz
            - 목적: ⭐ 4대→3대 감소 확인
        
        Phase 8 (660-720초, 1분): 정상 복귀 (40→41.5°C)
            - T6: 40°C → 41.5°C (약간 상승)
            - 예상: 48Hz 유지, 3대
        
        Phase 9 (720-810초, 1.5분): 극저온 진입 (41.5→38°C)
            - T6: 41.5°C → 38°C (매우 낮음)
            - 예상: 40Hz 재도달, 3대
        
        Phase 10 (810-900초, 1.5분): 40Hz 유지 → 3대→2대 감소
            - T6: 38°C 유지
            - 예상: 40Hz 10초 → 3대 → 2대 감소
            - 주파수: 40Hz → 48Hz
            - 목적: ⭐ 3대→2대 감소 확인 (최소 대수)
        
        Phase 11 (900-990초, 1.5분): 최종 정상 복귀 (38→43°C)
            - T6: 38°C → 43°C (목표 복귀)
            - 예상: 피드백 제어, 2대 유지
        """
        noise = np.random.normal(0, 0.2)
        ...
        # Phase 3 (150-300초): 60Hz 유지 (정상 대수 증설: 3→4대, Priority 4)
        elif t <= 300:
            t6_temp = 44.5  # 44.5°C 유지 → 60Hz 10초 → 4대 증설 (45°C 미만!)
        
        # Phase 4 (300-390초): 극한 근접 (선제 대응 테스트)
        elif t <= 390:
            t6_temp = 44.5 + ((t - 300) / 90.0) * 1.5  # 44.5 → 46°C
        
        # Phase 5 (390-480초): 온도 하강
        elif t <= 480:
            t6_temp = 46.0 - ((t - 390) / 90.0) * 3.0  # 46 → 43°C
        
        # Phase 6 (480-570초): 저온 진입
        elif t <= 570:
            t6_temp = 43.0 - ((t - 480) / 90.0) * 3.0  # 43 → 40°C
        
        # Phase 7 (570-660초): 40Hz 유지 (대수 감소: 4→3대)
        elif t <= 660:
            t6_temp = 40.0  # 40°C 유지 → 40Hz 10초 → 3대 감소
        
        # Phase 8 (660-720초): 정상 복귀
        elif t <= 720:
            t6_temp = 40.0 + ((t - 660) / 60.0) * 1.5  # 40 → 41.5°C
        
        # Phase 9 (720-810초): 극저온 진입
        elif t <= 810:
            t6_temp = 41.5 - ((t - 720) / 90.0) * 3.5  # 41.5 → 38°C
        
        # Phase 10 (810-900초): 40Hz 유지 (대수 감소: 3→2대)
        elif t <= 900:
            t6_temp = 38.0  # 38°C 유지 → 40Hz 10초 → 2대 감소
        
        # Phase 11 (900-990초): 최종 정상 복귀
        elif t <= 990:
            t6_temp = 38.0 + ((t - 900) / 90.0) * 5.0  # 38 → 43°C
